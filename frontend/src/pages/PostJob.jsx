...
const PostJob = () => {
  ...
  return (
    <div className="space-y-6 md:space-y-10">
      <div className="max-w-3xl mx-auto bg-white dark:bg-[#1E293B] shadow-xl dark:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] rounded-3xl md:rounded-[3rem] overflow-hidden border border-slate-100 dark:border-slate-600/50 transition-colors duration-300">
        <div className="bg-gradient-to-r from-indigo-500 to-violet-600 p-8 md:p-12 text-white relative">
          <h2 className="text-lg md:text-2xl lg:text-3xl font-black tracking-tight">{isEditMode ? "Edit Job Post" : "Post a New Job"}</h2>
          <p className="mt-2 md:mt-3 text-indigo-100 font-black uppercase tracking-widest text-[9px] md:text-[10px]">
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-10 space-y-6 md:space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
             ...
             <div className="space-y-4">
              <label className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 ml-1 transition-colors">Category</label>
              <div className="relative group/input">
                <select
                  name="category"
              ...
              {formData.category === "Other" && (
                <input
                  name="customCategory"
                  required
                  className="w-full mt-3 px-6 py-5 bg-slate-50 dark:bg-[#0F172A] border border-slate-200 dark:border-slate-600/50 focus:border-indigo-500/50 rounded-2xl outline-none font-bold transition text-slate-900 dark:text-[#E2E8F0] placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-inner"
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 ml-1 transition-colors">Job Description</label>
            <textarea
              name="description"
              required
              rows="8"
              className="w-full p-8 bg-slate-50 dark:bg-[#0F172A] border border-slate-200 dark:border-slate-600/50 focus:border-indigo-500/50 rounded-[2rem] outline-none font-medium text-slate-800 dark:text-slate-300 leading-relaxed transition placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-inner resize-none"
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider transition-colors">Skills Required</label>
            <div className="flex gap-2">
              <input
                name="skillInput"
                className="flex-1 px-4 py-3 bg-slate-50 dark:bg-[#0F172A] border border-slate-200 dark:border-slate-600 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition text-slate-900 dark:text-slate-200 placeholder:text-slate-400"
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {formData.skillsRequired.map((skill) => (
                <span key={skill} className="bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 border border-indigo-500/30 transition-colors">
                  {skill}
                  <X className="h-4 w-4 cursor-pointer hover:text-red-600 dark:hover:text-red-400 transition" onClick={() => removeSkill(skill)} />
                </span>
              ))}
            </div>
          </div>
